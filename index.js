...
const argv = yargs
  .option('security', {
	...
	.option('output', {
		description: 'example for python, \"-o \"',
		alias: ['S', 'src', 'in'],
		type: 'boolean'
  })
  .help()
...
if (argv.language == 'python') {
console.log(argv.security + 2);
let txt = "let txt = fs.readFileSync('text.txt', 'utf-8')"
let fp3 = "./encode.js"
	fs.appendFile(fp3, txt, function(error) {
		if (error) return log.error(error);
	})
let fp4 = "./decode.js"
	fs.appendFile(fp4, txt, function(error) {
		if (error) return log.error(error);
	})
...
if (argv.language == 'ahk') {
console.log(argv.security + 3);
let txt = "let txt = fs.readFileSync('text.txt', 'utf-8')"
let fp3 = "./encode.js"
	fs.appendFile(fp3, txt, function(error) {
		if (error) return log.error(error);
...
if (e == '0' || '1') {
	setTimeout(repp, 1000);
}
function repp() {
var rand = randomString(argv.s + 3, '¡€£¤¥ª°¹²³·®÷ǁ¼½¾⁵¿×ȼˢ⁶⁷⁸⁹⁺ø•◦∙‣⁃∞₹₨₱₩฿₫₪');
let txt = ".replace(/[\""+`${characters[e]}`+"\"]/g, \""+`¢${rand}¢`+"\")"
let fp3 = "./encode.js"
	fs.appendFile(fp3, txt, function(error) {
		if (error) return log.error(error);
...
setTimeout(docrap, 4000);
function docrap() {
let txt3 = `;\nlet path = \"./encoded.txt\"\nfs.appendFile(path, txt, function(error) {\n		if (error) return log.error(error);\n	})`
let fp5 = "./encode.js"
	fs.appendFile(fp5, txt3, function(error) {
		if (error) return log.error(error);
	})
let txt4 = `;\nlet path = \"./decoded.txt\"\nfs.appendFile(path, txt, function(error) {\n		if (error) return log.error(error);\n	})`
let fp6 = "./decode.js"
	fs.appendFile(fp6, txt4, function(error) {
		if (error) return log.error(error);
	})
}